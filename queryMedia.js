...
const groupCssMediaQueries = postcssPlugin("css-media-queries-plugin", () => root => {
  const medias = {};
  ...
  const sortedListRules = [onlyMinRules, onlyMaxRules, intervalRules, otherRules];

  for (const rules of sortedListRules) {
    for (const _ref of rules) {
      const {
        params,
        mediaQueryRules
      } = _ref;
